...
async function request<T>(
  ...
  const { token, isFormData, ...requestConfig } = config;
...
export const api = {
  ...
  users: {
    ...
    getAll: (token: string) => request<User[]>("users/", { token }).then(ensureArray),
    get: (id: number, token: string) => request<User>(`users/${id}/`, { token }),
    delete: (id: number, token: string) => request<{}>(`users/${id}/delete/`, {
      method: "DELETE",
      token,
    }),
    ...
    getReviews: (userId: number, token: string) =>
      request<Review[]>(`users/${userId}/reviews/`, { token }).then(ensureArray),

    getMarks: (userId: number, token: string) =>
      request<Mark[]>(`users/${userId}/marks/`, { token }).then(ensureArray),

    assignMark: (userId: number, data: AssignMarkData, token: string) =>
    ...
    updateMark: (userId: number, markId: number, data: UpdateMarkData, token: string) =>
    ...
    deleteMark: (userId: number, markId: number, token: string) =>
  ...
  reports: {
    getAll: (token: string, queryParams?: Record<string, string>) => {
      let url = "/reports/all";
      if (queryParams) {
      ...
      return request<Report[]>(url, { token }).then(ensureArray);
    },
